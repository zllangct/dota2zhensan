...
function zhongjiaoyi_02(keys)
    ...
    if difference_vector:Length() > 700 then 
        target_point = origin_point + (target_point - origin_point):Normalized() * 600
    end
   caster:SetOrigin(target_point)
   FindClearSpaceForUnit(caster, target_point, false)
 -----------------------------
    local p_end1 = 'particles/econ/items/shadow_fiend/sf_fire_arcana/sf_fire_arcana_shadowraze.vpcf'
    local p_index1 = ParticleManager:CreateParticle(p_end1, PATTACH_CUSTOMORIGIN, caster)
